...
for i in tqdm(range(low_num+1,up_num+1)):
   
	temp_data = data.copy()
#     temp_data = temp_data[temp_data['date']

	if low_num < 9:
		print('Processing: ',temp_data[(temp_data['date'] == '2016-05-2'+str(i+1))]['date'].unique())
	else:
		print('Processing: ',temp_data[(temp_data['date'] == '2016-06-'+str(i-8).zfill(2))]['date'].unique())


	print('\n Making features ----------------- \n')
	
	temp_data = time_fe(temp_data)
	temp_data = lag_fe(temp_data,i)
	...
	if low_num < 9:
		test_row = temp_data[(temp_data['date'] == '2016-05-2'+str(i+1))].drop(drop_feats,axis=1)
	else:
		test_row = temp_data[(temp_data['date'] == '2016-06-'+str(i-8).zfill(2))].drop(drop_feats,axis=1)
	...
	print('\n Test Num features: ', test_row.shape[1])
	print('\n Test Features: ', test_row.columns.values)
	...
	# model estimation
	pre_model = lgb.train(params, train_set, num_boost_round = 20000, early_stopping_rounds = 50, 
					  valid_sets = [train_set, val_set], verbose_eval = 100, feval= wrmsse)

	val_pred = pre_model.predict(x_val, num_iteration=pre_model.best_iteration)
	...
	y_train = temp_data[(temp_data['date'] > lower_date) & (temp_data['date'] <= upper_date)]['demand']
	x_train = temp_data[(temp_data['date'] > lower_date) & (temp_data['date'] <= upper_date)].drop(drop_feats,axis=1)
	...
	if low_num < 9:
		test.loc[test[test['date'] == '2016-05-2'+str(i+1)].index,'demand'] = y_pred
	else:
		test.loc[test[test['date'] == '2016-06-'+str(i-8).zfill(2)].index,'demand'] = y_pred
...
predictions.columns = ['id'] + ['F' + str(i) for i in range(low_num+1,up_num+1)]
